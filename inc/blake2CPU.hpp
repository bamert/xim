...
/*  The header that we process only contains data within the first 80bytes, i.e. m[0]...m[9]. 
 *  this means m[10]...m[15] are always zero.
 */
#define ROUNDSWITHZEROS(m,nonce){ \
        B2B_G( 0, 4,  8, 12, m[0], m[1]);\
        B2B_G( 1, 5,  9, 13, m[2], m[3]);\
        B2B_G( 2, 6, 10, 14, nonce, m[5]);\
        B2B_G( 3, 7, 11, 15, m[6], m[7]);\
        ...
        B2B_G( 3, 4,  9, 14, 0, 0);\
         B2B_G( 0, 4,  8, 12, 0, 0);\
        B2B_G( 1, 5,  9, 13, nonce, m[8]);\
        B2B_G( 2, 6, 10, 14, m[9], 0);\
        B2B_G( 3, 7, 11, 15, 0, m[6]);\
        B2B_G( 0, 5, 10, 15, m[1], 0);\
        B2B_G( 1, 6, 11, 12, m[0], m[2]);\
        B2B_G( 2, 7,  8, 13, 0, m[7]);\
        B2B_G( 3, 4,  9, 14, m[5], m[3]);\
         B2B_G( 0, 4,  8, 12, 0, m[8]);\
        B2B_G( 1, 5,  9, 13, 0, m[0]);\
        B2B_G( 2, 6, 10, 14, m[5], m[2]);\
        B2B_G( 3, 7, 11, 15, 0, 0);\
        B2B_G( 0, 5, 10, 15, 0, 0);\
        B2B_G( 1, 6, 11, 12, m[3], m[6]);\
        ...
        B2B_G( 3, 7, 11, 15, 0, 0);\
        B2B_G( 0, 5, 10, 15, m[2], m[6]);\
        B2B_G( 1, 6, 11, 12, m[5], 0);\
        B2B_G( 2, 7,  8, 13, nonce, m[0]);\
        B2B_G( 3, 4,  9, 14, 0, m[8]);\
         B2B_G( 0, 4,  8, 12, m[9], m[0]);\
        B2B_G( 1, 5,  9, 13, m[5], m[7]);\
        B2B_G( 2, 6, 10, 14, m[2], nonce);\
        B2B_G( 3, 7, 11, 15, 0, 0);\
        B2B_G( 0, 5, 10, 15, 0, m[1]);\
        B2B_G( 1, 6, 11, 12, 0, 0);\
        B2B_G( 2, 7,  8, 13, m[6], m[8]);\
        B2B_G( 3, 4,  9, 14, m[3], 0);\
         B2B_G( 0, 4,  8, 12, m[2], 0);\
        B2B_G( 1, 5,  9, 13, m[6], 0);\
        B2B_G( 2, 6, 10, 14, m[0], 0);\
        B2B_G( 3, 7, 11, 15, m[8], m[3]);\
        B2B_G( 0, 5, 10, 15, nonce, 0);\
        B2B_G( 1, 6, 11, 12, m[7], m[5]);\
        B2B_G( 2, 7,  8, 13, 0, 0);\
        B2B_G( 3, 4,  9, 14, m[1], m[9]);\
         B2B_G( 0, 4,  8, 12, 0, m[5]);\
        B2B_G( 1, 5,  9, 13, m[1], 0);\
        B2B_G( 2, 6, 10, 14, 0, 0);\
        B2B_G( 3, 7, 11, 15, nonce, 0);\
        B2B_G( 0, 5, 10, 15, m[0], m[7]);\
        B2B_G( 1, 6, 11, 12, m[6], m[3]);\
        B2B_G( 2, 7,  8, 13, m[9], m[2]);\
        B2B_G( 3, 4,  9, 14, m[8], 0);\
         B2B_G( 0, 4,  8, 12, 0, 0);\
        B2B_G( 1, 5,  9, 13, m[7], 0);\
        B2B_G( 2, 6, 10, 14, 0, m[1]);\
        B2B_G( 3, 7, 11, 15, m[3], m[9]);\
        B2B_G( 0, 5, 10, 15, m[5], m[0]);\
        B2B_G( 1, 6, 11, 12, 0, nonce);\
        B2B_G( 2, 7,  8, 13, m[8], m[6]);\
        B2B_G( 3, 4,  9, 14, m[2], 0);\
         B2B_G( 0, 4,  8, 12, m[6], 0);\
        B2B_G( 1, 5,  9, 13, 0, m[9]);\
        B2B_G( 2, 6, 10, 14, 0, m[3]);\
        B2B_G( 3, 7, 11, 15, m[0], m[8]);\
        B2B_G( 0, 5, 10, 15, 0, m[2]);\
        B2B_G( 1, 6, 11, 12, 0, m[7]);\
        B2B_G( 2, 7,  8, 13, m[1], nonce);\
        B2B_G( 3, 4,  9, 14, 0, m[5]);\
         B2B_G( 0, 4,  8, 12, 0, m[2]);\
        B2B_G( 1, 5,  9, 13, m[8], nonce);\
        B2B_G( 2, 6, 10, 14, m[7], m[6]);\
        B2B_G( 3, 7, 11, 15, m[1], m[5]);\
        B2B_G( 0, 5, 10, 15, 0, 0);\
        B2B_G( 1, 6, 11, 12, m[9], 0);\
        B2B_G( 2, 7,  8, 13, m[3], 0);\
        B2B_G( 3, 4,  9, 14, 0, m[0]);\
        B2B_G( 0, 4,  8, 12, m[0], m[1]);\
        B2B_G( 1, 5,  9, 13, m[2], m[3]);\
        B2B_G( 2, 6, 10, 14, nonce, m[5]);\
        B2B_G( 3, 7, 11, 15, m[6], m[7]);\
 ...
 public:
  ...
  bool hashRange( const unsigned char* header, uint32_t startNonce, uint32_t endNonce,
    ...
    for (uint32_t k = startNonce; k < endNonce; k++) {
      ...
      //we only care about the first 64 bits of the hash 7640891576956012808 XOR 16842784 = 0x6A09E667F2BDC928
